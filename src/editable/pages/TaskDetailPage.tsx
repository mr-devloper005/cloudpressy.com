  ...
  return (
    <section className="bg-white">
      <header className="relative overflow-hidden bg-[var(--slot4-dark-bg)] text-white">
        <img src={images[0] || 'https://images.unsplash.com/photo-1494412651409-8963ce7935a7?auto=format&fit=crop&w=1800&q=80'} alt="" className="absolute inset-0 h-full w-full object-cover opacity-45" />
        <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(25,56,69,.96),rgba(25,56,69,.62))]" />
        <div className="relative mx-auto max-w-[1180px] px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
          <BackLink task={task} />
          <p className="mt-10 text-xs font-black uppercase tracking-[0.22em] text-[var(--slot4-accent)]">{categoryOf(post, 'Media distribution')}</p>
          <h1 className="logistics-display mt-5 max-w-6xl text-6xl leading-[0.92] sm:text-7xl lg:text-[6.5rem]">{post.title}</h1>
        </div>
      </header>
...
function RelatedPanel({ task, post, related, compact = false }: { task: TaskKey; post: SitePost; related: SitePost[]; compact?: boolean }) {
  ...
  return (
    <aside className="min-w-0 space-y-5">
      {!compact && task !== 'mediaDistribution' && task !== 'article' ? (
...
function RelatedCard({ task, post }: { task: TaskKey; post: SitePost }) {
  ...
  return (
    <Link href={buildPostUrl(task, post.slug)} className="group flex gap-3 border-t border-black/15 py-3 transition hover:text-[var(--slot4-accent)]">
      {image && task !== 'sbm' ? <img src={image} alt="" className="h-20 w-20 shrink-0 rounded-md object-cover" /> : <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-md bg-[var(--slot4-dark-bg)] text-white"><FileText className="h-6 w-6" /></div>}
...
function EditableComments({ slug, comments }: { slug: string; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  return (
    <section className="mt-12 rounded-[1rem] border border-black/10 bg-white p-5">

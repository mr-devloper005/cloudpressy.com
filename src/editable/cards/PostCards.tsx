...
export function RailPostCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className={`group ${dc.layout.minRailCard} block rounded-[1rem] border border-black/10 bg-white p-5 ${dc.motion.lift}`}>
      ...
      <h3 className="mt-10 line-clamp-3 text-2xl font-black leading-[1.05] tracking-[-.025em] group-hover:text-[var(--slot4-accent)]">{post.title}</h3>
      <p className="mt-4 line-clamp-4 text-sm font-semibold leading-6 text-[var(--slot4-muted-text)]">{getEditableExcerpt(post, 150)}</p>
      <span className="mt-8 inline-flex items-center gap-2 text-xs font-black uppercase tracking-[.14em] text-[var(--slot4-dark-bg)]">Open brief <ArrowRight className="h-4 w-4" /></span>
    </Link>
  )
}
...
export function CompactIndexCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className="group grid min-w-0 grid-cols-[46px_1fr] gap-4 border-t border-black/10 py-5 first:border-t-0">
